...
import { db } from "@/db";
import { documents, blocks, workspaces, operations, users, documentCollaborators, workspaceMembers, integrationAccounts, documentSnapshots } from "@/db/schema";
...
export const documentsRouter = createTRPCRouter({
  ...
  getDocumentVersions: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      // 验证文档权限
      console.log(`[getDocumentVersions] Fetching versions for documentId: ${input.documentId}`);
      const [documentRecord] = await db
        .select()
        .from(documents)
        .leftJoin(workspaces, eq(documents.workspaceId, workspaces.id))
      ...
      const snapshots = await db
        .select({
          ...
          creatorImageUrl: users.imageUrl,
        })
      ...
      return {
        versions: snapshots.map(s => ({
            ...
            creator: s.creatorId ? {
                id: s.creatorId,
                name: s.creatorName,
                imageUrl: s.creatorImageUrl,
            } : null
        })),
        currentVersion: snapshots[0]?.version || 0,
      };
    }),
  ...
  restoreDocumentVersion: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const blocksToRestore = targetSnapshot.blocksSnapshot as typeof blocks.$inferSelect[];
      ...
      await db.transaction(async (tx) => {
        ...
        await tx.insert(documentSnapshots).values({
          ...
          reason: `Auto-save before restore to v${input.version}`,
          createdBy: ctx.user.id,
          blockCount: currentBlocks.length,
        });

        // 2. 删除当前所有 Blocks
        await tx.delete(blocks).where(eq(blocks.documentId, input.documentId));
        ...
        if (blocksToRestore && blocksToRestore.length > 0) {
          ...
          // 4. 重建 Yjs 状态并更新文档
          // 必须更新 yjsState，否则编辑器加载时会使用旧的（未来的）状态
          const doc = new Y.Doc();
          const yBlocks = doc.getArray("blocks");

          // 按位置排序
          const sortedBlocks = [...blocksToRestore].sort((a, b) => a.position - b.position);
          ...
          sortedBlocks.forEach((block) => {
            ...
            if (block.type === "code") {
              ...
              const items = content.todo?.items || [];
              text = items
                .map((item: { checked: boolean; text: string }) => `${item.checked ? "[x] " : "[ ] "}${item.text}`)
                .join("\n");
            } else {
          ...
          await tx
            .update(documents)
            .set({ yjsState: base64, updatedAt: new Date() })
            .where(eq(documents.id, input.documentId));
        } else {
            // 如果没有 block，清空 yjsState
            const doc = new Y.Doc();
            const state = Y.encodeStateAsUpdate(doc);
            const base64 = Buffer.from(state).toString("base64");
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await db.transaction(async (tx) => {
        ...
        const newBlocks = items.map((item, index) => {
          ...
          if (type === "code") {
            blockContent = { code: { content: text, language: "javascript" } };
          } else if (type === "list") {
            blockContent = { list: { items: text ? text.split("\n") : [] } };
          } else if (type === "todo") {
            ...
            const items = lines.map((line) => {
              ...
              let txt = trim;
              if (checked) txt = trim.slice(4);
              else if (trim.startsWith("[ ] ")) txt = trim.slice(4);
              return { text: txt, checked };
            });
            blockContent = { todo: { items } };
          } else {
          ...
          return {
            id,
            documentId: input.documentId,
            type: type as typeof blocks.$inferInsert.type,
        ...
        // 3. 创建快照 (Auto-save snapshot)
        // 为了避免快照过多，这里可以添加逻辑：比如只有当上一个快照是 10 分钟前才创建
        // 但目前为了确保版本回退功能可用，我们先每次保存都创建（前端已有 debounce）
        const [lastSnapshot] = await tx
          .select()
          .from(documentSnapshots)
          .where(eq(documentSnapshots.documentId, input.documentId))
